...
async def test_me(ac: AsyncClient):
    response = await ac.post("/app/Authentication/SignIn", json={"username": "admin", "password": "admin"})
    token = response.json()["access_token"]
    response = await ac.get("/app/Accounts/Me", headers={
...
async def test_update(ac: AsyncClient):
    response = await ac.post("/app/Authentication/SignIn", json={"username": "manager", "password": "manager"})
    token = response.json()["access_token"]
    response = await ac.put("/app/Accounts/Update", json={
        "firstName": "new_name",
        "lastName": "new_name",
        "password": "manager"
    }, headers={
...
async def test_get_accounts(ac: AsyncClient, users_data: dict):
    ...
    response = await ac.get('/app/Accounts', params={
        'from': 0,
        'count': 10
    }, headers={
...
async def test_post_accounts(ac: AsyncClient, users_data):
    ...
    response = await ac.post('/app/Accounts', json={
        "firstName": "string",
        "lastName": "string",
        "username": "string",
        "password": "string",
...
async def test_put_accounts(ac: AsyncClient, users_data):
    ...
    response = await ac.put(f'/app/Accounts/{user.id}', json={
        "firstName": "new_name",
        "lastName": "new_name",
...
async def test_delete_accounts(ac: AsyncClient, users_data):
    ...
    response = await ac.delete(f'/app/Accounts/{user.id}', headers={
        "Authorization": f"Bearer {users_data['token']}"
    })

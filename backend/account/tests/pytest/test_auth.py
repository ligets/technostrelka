...
async def test_register(ac: AsyncClient):
    response = await ac.post('/app/Authentication/SignUp', json={
        "firstName": "test_user",
        "lastName": "test_user",
        "username": "test_user",
        "password": "test_user"
...
async def test_login(ac: AsyncClient):
    global login_data
    response = await ac.post('/app/Authentication/SignIn', json={
        'username': 'test_user',
        'password': 'test_user'
    })
...
async def test_refresh(ac: AsyncClient):
    global login_data
    response = await ac.post('/app/Authentication/Refresh', json={
...
async def test_logout(ac: AsyncClient):
    global login_data
    response = await ac.put('/app/Authentication/SignOut', headers={
